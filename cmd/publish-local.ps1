...
dotnet build "$repoRoot\AN_YeetCode.sln" -c $configuration @versionProps
...
dotnet pack "$repoRoot\YeetJson.lib\YeetJson\YeetJson.csproj" -c $configuration --no-build @versionProps
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

Write-Host "`n[3/5] Packing YeetCode library..." -ForegroundColor Green
dotnet pack "$repoRoot\YeetCode.lib\YeetCode\YeetCode.csproj" -c $configuration --no-build @versionProps
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

# Pack the CLI tool
Write-Host "`n[4/5] Packing CLI tool..." -ForegroundColor Green
dotnet pack "$repoRoot\YeetCode.CLI\YeetCode.CLI.csproj" -c $configuration --no-build @versionProps
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

# Pack the MSBuild task
Write-Host "`n[5/5] Packing MSBuild task..." -ForegroundColor Green
dotnet pack "$repoRoot\YeetCode.MSBuild\YeetCode.MSBuild.csproj" -c $configuration --no-build @versionProps
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
